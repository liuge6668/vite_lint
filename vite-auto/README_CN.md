...
- **多种布局模式**：
  ...
  - **侧边布局**：全高侧边栏，底部工具栏设计（收起时hover显示菜单）
  - **顶部布局**：水平菜单栏布局，鼠标悬停展开菜单
- **侧边栏展开收起**：平滑动画、防抖处理、同步过渡效果
- **面包屑导航**：基于路由层级自动生成的面包屑导航
- **主题切换**：亮色/暗色主题支持
- **响应式设计**：移动端友好的布局适配
...
### 📋 产品规划
- **多步骤流程**：市场调研、需求分析、规划制定的三步流程
- **步骤导航**：可视化步骤指示器，支持点击跳转
- **表单管理**：各步骤包含相应的表单输入
- **面包屑导航**：基于嵌套路由的面包屑导航

### 🧩 组件展示
- **组件库展示**：展示 Element Plus 常用组件的使用方法
- **交互式演示**：按钮、输入框、表格等组件的多种用法
- **代码示例**：直观的组件效果展示和配置说明
...
#### 页面导航
- **面包屑导航**：基于当前位置自动生成的面包屑导航
- **首页**：主要仪表板，包含统计数据
- **关于**：信息展示页面
- **仪表板**：管理员专属页面，包含详细分析数据
- **产品规划**：多步骤产品规划流程（市场调研、需求分析、规划制定）
- **工具**：实用工具集合（侧边栏子菜单中）
  - 计算器：基础计算功能
  - 日历：日程查看工具
  - 组件展示：Element Plus 组件使用演示
    - Button 按钮：基础用法、尺寸、状态等
    - Input 输入框：基础输入、密码框、带图标等
    - Table 表格：基础表格、操作表格等
    - Dialog 对话框：基础对话框、自定义对话框等
    - Select 选择器：单选、多选、可过滤等
    - Switch 开关：基础用法、自定义样式等
...
│   ├── Login/       # 认证页面
│   ├── Dashboard/   # 管理仪表板
│   ├── ProductPlanning/ # 产品规划多步骤流程
│   ├── ComponentsShowcase/ # 组件展示页面
│   │   ├── index.vue      # 组件展示主页
│   │   ├── Button.vue     # 按钮组件展示
│   │   ├── Input.vue      # 输入框组件展示
│   │   ├── Table.vue      # 表格组件展示
...
{
  "username": "admin",    // 或其他用户名获得用户角色
  "password": "123456"    // 统一的演示密码
}
